...
import { createMentorSlot, getMySlots } from "../services/mentorService";
    ...
    <DashboardLayout navItems={ALUMNI_NAV} roleLabel="Alumni" title="My Slots" userName={user?.name || "User"}>
      <div className="glass-card p-6">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-display text-lg font-semibold text-white">Add availability</h2>
        </div>
          ...
          <div className="glass-card overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full min-w-[480px] text-left text-sm">
                ...
                <tbody>
                  {slots.map((slot) => (
                    <tr key={slot.id} className="border-b border-white/5 last:border-0">
                      <td className="px-6 py-3.5 text-slate-300">{new Date(slot.startTime).toLocaleString()}</td>
                      <td className="px-6 py-3.5 text-slate-300">{new Date(slot.endTime).toLocaleString()}</td>
                      <td className="px-6 py-3.5">
                        <span
                          className={
                            "rounded-full px-2.5 py-1 text-xs font-semibold " +
                            (slot.booked ? "bg-emerald-500/10 text-emerald-400" : "bg-white/5 text-slate-400")
                          }
                        >
